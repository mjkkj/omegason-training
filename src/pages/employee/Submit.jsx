...
import { W, Card, H, T, Btn, Tag, Check, Field, Ico } from '../../design-system'
...
export default function Submit() {
  ...
  useEffect(() => {
    ...
    fetchMySubmissions(currentUser.id).then(data => {
      ...
      if (sub) { setExistingSub(sub); setNote(sub.note || '') }
    })
  }, [currentUser?.id, id])
  ...
  const handleFile = (f) => {
    ...
    if (!f.name.match(/\.(html|htm)$/i)) { setError('Chỉ chấp nhận file .html'); return }
    setError(null)
  ...
  const handleSubmit = async () => {
    ...
    try {
      await upsertSubmission({
        ...
        taskId: id,
        fileName: file?.name || existingSub?.file_name,
        fileContent: fileContent || existingSub?.file_content,
        fileSize: file?.size || existingSub?.file_size,
        note,
      })
      setSubmitted(true)
      setTimeout(() => navigate(`/emp/task/${id}`), 1400)
    } catch (err) {
  ...
  if (submitted) return (
    <Shell role="emp" title="Nộp báo cáo" sub={`Task ${id}`} body={W.panel2}>
  ...
  const fileToShow = file || (existingSub ? { name: existingSub.file_name, size: existingSub.file_size } : null)

  return (
    <Shell role="emp" title="Nộp báo cáo" sub={`Task ${id} · ${task.name}`} body={W.panel2}>
          ...
          <div style={{ padding:22, display:'flex', flexDirection:'column', gap:18 }}>
            ...
            <div
              ...
              style={{ border:`1.5px dashed ${dragging ? W.acc : W.accLine}`, borderRadius:12,
                background: W.accSoft, padding:'26px 20px', textAlign:'center', cursor:'pointer' }}>
            ...
            {fileToShow && (
              <Card pad={12} fill={W.panel} style={{ display:'flex', alignItems:'center', gap:11 }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:12.5, fontWeight:700, fontFamily: W.mono,
                    overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{fileToShow.name}</div>
                  <T size={11} style={{ marginTop:2 }}>{formatSize(fileToShow.size)} · {file ? 'vừa chọn' : 'đã nộp trước'}</T>
                </div>
                {file && <Tag tone="done">✓ Hợp lệ</Tag>}
              </Card>
            )}

            {(fileContent || existingSub?.file_content) && (
              <Card pad={0} style={{ overflow:'hidden' }}>
                <div style={{ padding:'8px 14px', borderBottom:`1px solid ${W.line2}`, background: W.panel,
                  display:'flex', gap:8 }}>
                  <Ico name="eye" s={13} c={W.ink3} />
                  <T size={11.5} weight={600}>Xem trước</T>
                </div>
                <iframe srcDoc={fileContent || existingSub?.file_content}
                  sandbox="allow-same-origin" style={{ width:'100%', height:220, border:'none', display:'block' }} />
              </Card>
            )}
            ...
            <div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px 16px' }}>
                {SELFCHECK.map((c, i) => (
                  <Check key={i} on={checks[i]} label={c} style={{ cursor:'pointer' }}
