...
function timeAgo(iso, t) {
  ...
  if (d < 60) return `${d}${t(' phút trước')}`
  const h = Math.floor(d / 60)
  if (h < 24) return `${h}${t(' giờ trước')}`
  return `${Math.floor(h/24)}${t(' ngày trước')}`
}
...
export default function Queue() {
  ...
  return (
    <Shell role="mgr" title={t('Hàng chờ chấm bài')} sub={`${queue.length} ${t('báo cáo đang chờ')}`}
      body={W.panel} pad={18}>
      ...
      {!loading && queue.length === 0 ? (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'60%', gap:12 }}>
          ...
          <H size={18}>{t('Hàng chờ trống!')}</H>
          <T size={13.5}>{t('Tất cả báo cáo đã được chấm.')}</T>
          <Link to="/mgr/stats"><Btn kind="soft" size="md">{t('Xem thống kê')}</Btn></Link>
        </div>
      ) : (
        <div style={{ display:'flex', gap:16, height:'100%' }}>
          ...
          <div style={{ width:272, flexShrink:0, display:'flex', flexDirection:'column', gap:8, overflowY:'auto' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:2 }}>
              <T size={11} mono c={W.ink3}>{t('HÀNG CHỜ · ')}{queue.length}</T>
            </div>
            {queue.map(q => {
              const e = q.profiles
                  ...
                  <Card pad={12} fill={sel?W.accSoft:'#fff'} line={sel?W.accLine:W.line}
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontSize:12.5, fontWeight:600, overflow:'hidden', textOverflow:'ellipsis',
                        whiteSpace:'nowrap' }}>{e?.name || t('Nhân viên')}</div>
                      <div style={{ fontSize:10.5, color: W.ink3 }}>
                        {moduleLabel(TASKS.find(tk => tk.id === q.task_id))} · {timeAgo(q.submitted_at, t)}
                      </div>
                    </div>
                  </Card>
                </Link>
            ...
            <div style={{ flex:1, minWidth:0, display:'flex', gap:14 }}>
              <Card pad={0} style={{ flex:1, minWidth:0, display:'flex', flexDirection:'column', overflow:'hidden' }}>
                <div style={{ padding:'11px 16px', borderBottom:`1px solid ${W.line2}`,
                  ...
                  <T size={11.5} c={W.ink3}>{timeAgo(selected.submitted_at, t)}</T>
                </div>
              ...
              <div style={{ width:300, flexShrink:0, display:'flex', flexDirection:'column', gap:12 }}>
                <Card pad={16}>
                  <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:12 }}>
                    <T size={11} mono c={W.ink3}>{t('CHECKLIST · ')}{checked.length}/{items.length}</T>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                    ...
                    {items.length === 0 && <T size={12.5} c={W.ink3}>{t('Module này chưa có checklist.')}</T>}
                  </div>
                </Card>

                <Card pad={14}>
                  <Field label={t('Nhận xét cho nhân viên')}>
                    <textarea value={feedback} onChange={e => setFeedback(e.target.value)}
                      placeholder={t('Góp ý giúp nhân viên cải thiện các mục còn thiếu…')} rows={4}
